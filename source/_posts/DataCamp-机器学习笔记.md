...
链接中的csv包含的数据是Titanic乘客的数据,大体数据在文末有显示。
```python
...
- **3、 value_counts()**
用于统计某一项的数据情况。例如train中包含有乘客生还信息，即"Survived"一项，1表示活，0表示die。下面用value_counts()来统计生还情况。
```python
In [2]: train['Survived'].value_counts()
Out[2]: 
0    549
1    342
...
value_counts()中还有** normalize **参数可以设定，若设定为 **True**，则会将结果以比率的形式显示
```python
...
**三、 Intro to decision trees**

> 学习使用 scikit-learn library
